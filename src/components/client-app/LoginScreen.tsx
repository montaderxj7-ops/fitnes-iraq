    ...
    <div className="min-h-full p-6 relative bg-[#050505] text-white flex flex-col overflow-x-hidden">
      <div className="z-10 max-w-md w-full mx-auto flex-1 flex flex-col pt-4">
        <div className="flex flex-col mb-10">
          ...
          >
            <h2 className="text-3xl sm:text-4xl font-black tracking-tight text-white mb-2">{t('auth.login')}</h2>
            <p className="text-gray-400 text-sm sm:text-base font-medium">{t('auth.welcomeBack')}</p>
          </motion.div>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <div className="relative group">
            <label className="text-xs font-bold text-gray-400 mb-2 block px-1 group-focus-within:text-gray-300 transition-colors">{t('auth.email')}</label>
          ...
          <div className="relative group">
            <label className="text-xs font-bold text-gray-400 mb-2 block px-1 group-focus-within:text-gray-300 transition-colors">{t('auth.password')}</label>
            <div dir="ltr" className="relative bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl overflow-hidden transition-all group-focus-within:bg-white/10 group-focus-within:border-white/20 group-focus-within:shadow-[0_0_20px_rgba(255,255,255,0.05)]">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
          ...
          <div className="pt-6">
            ...
            <div className="relative flex items-center py-4">
              ...
              <span className="flex-shrink-0 mx-4 text-gray-500 text-sm font-medium">{t('auth.orLoginWith')}</span>
              <div className="flex-grow border-t border-white/10"></div>
            </div>

            <button
              type="button"
              onClick={() => alert(t('auth.googleComingSoon'))}
